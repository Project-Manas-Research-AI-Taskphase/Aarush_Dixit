...
#the entire graph code::::::::::::::: (Yeah I did not make this myself wth too much i care abt the math)
...
mesh_df  = pd.DataFrame({f1: xx.ravel(), f2: yy.ravel()})

# best linear config from the search
lin_mask = cv_results["param_svc__kernel"] == "linear"
lin_best = cv_results.loc[cv_results.loc[lin_mask, "mean_test_score"].idxmax()]
lin_C = lin_best["param_svc__C"]

# best rbf config from the search
rbf_mask = cv_results["param_svc__kernel"] == "rbf"
rbf_best = cv_results.loc[cv_results.loc[rbf_mask, "mean_test_score"].idxmax()]
rbf_C, rbf_gamma = rbf_best["param_svc__C"], rbf_best["param_svc__gamma"]

models = {
    "Linear — default\n(C=1)":           SVC(kernel="linear", random_state=42),
    "RBF — default\n(C=1, gamma=scale)": SVC(kernel="rbf", random_state=42),
    f"Linear — tuned\n(C={lin_C})":      SVC(kernel="linear", C=lin_C, random_state=42),
    f"RBF — tuned\n(C={rbf_C}, gamma={rbf_gamma})":
                                          SVC(kernel="rbf", C=rbf_C, gamma=rbf_gamma, random_state=42),
}
...
for ax, (title, clf) in zip(axes[1:5], models.items()):
    ...
    Z = pipe.decision_function(mesh_df ).reshape(xx.shape)
